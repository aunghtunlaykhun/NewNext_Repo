...
const RightSidebar = () => {
    ...
    const hotQuestions = [
      ...
      const popularTags = [
        ...
        {_id:'6',name:'react-query',questions:10},
      ]
  ...
  return (
    <section className='pt-36 custom-scrollbar background-light900_dark200 light-border sticky right-0 top-0 flex flex-col h-screen w-[350px] gap-6 overflow-y-auto border-l p-6 shadow-light-300 dark:shadow-none max-xl:hidden'>
        <div>
            <h3 className="h3-bold text-dark200_light900 ">hello</h3>
            <div className='mt-7 flex w-full flex-col gap-[30px]'>
                {
                    hotQuestions.map(({_id,title})=>(
                        <Link key={_id} href={ROUTES.PROFILE(_id)} className='flex cursor-pointer items-center justify-between gap-7'>
                            ...
                            <Image src="/icons/chevron-right.svg" alt="Chevron" className='invert-colors' width={20} height={20}/>
                        </Link>
                    ))
                }
            </div>
